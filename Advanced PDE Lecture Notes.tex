...
\begin{example}{Laplace equation with Dirichlet boundary conditions}{}
    Let \(\Omega \in \mathbb{R}^{n}\) be a bounded domain with \(\partial \Omega\) smooth.
    ...
    \begin{enumerate}
        ...
        \item In what sense is the understood (classical / weak)?
...
\begin{proof}{Application of Riesz representation theorem}
    ...
    \]
    But we obtained that \ref{eq:3.2} holds only for \(\varphi \in C^{\infty}_{0}\), not for \(\varphi \in W^{1, 2}_{0}(\Omega)\). However, \(W^{1, 2}_{0}(\Omega) = [C^{\infty}_{0}(\Omega)]_{W^{1, 2}}\). Then approximation arguments give that \(\forall \varphi \in H\), 
...
\begin{example}{Laplace equation with Neumann boundary conditions}{}
    ...
    \begin{equation}\label{eq:3.4}
        ...
        \end{cases}
    \end{equation}
    We cannot consider \(\varphi \in C^{\infty}_{0}(\Omega)\) as test functions, because the information about boundary conditions will be lost. Similarly,
    \[
